...
from .draw import draw_handler, jrrp_handler
...
rdhelp = on_startswith(".dhelp", priority=2, block=True)
...
drawcommand = on_startswith(".draw", priority=5, block=True)
jrrpcommand = on_startswith(".jrrp", priority=5, block=True)


@rdhelp.handle()
async def rdhelphandler(bot: Bot, event: Event):
    args = str(event.get_message())[6:].strip()
...
@drawcommand.handle()
async def drawhandler(bot: Bot, event: Event):
    args = str(event.get_message())[5:].strip().lower()
    for msg in draw_handler(args):
        await drawcommand.send(msg)


@jrrpcommand.handle()
async def jrrphandler(bot: Bot, event: Event):
    uid = event.get_user_id()
    for msg in jrrp_handler(uid):
        await jrrpcommand.send(msg)
